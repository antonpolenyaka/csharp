...
namespace APChiaPlotManager
{
    class Program
    {
        ...
        private const string _version = "v.1.0.1";
            ...
            while (!goExit)
            {
                ...
                if (!goExit)
                {
                    foreach (var originFolder in originFolders)
                    {
                        ...
                        {
                            Console.WriteLine($"{Now()} Original folder not exist: {originFolder}");
                            continue;
                        }
                        ...
                        if (fileNames != null)
                        {
                            foreach (var fileName in fileNames)
                            {
                                ...
                                {
                                    Console.WriteLine($"{Now()} File not exist: {fileName}");
                                    continue;
                                }
                                var fi = new FileInfo(fileName);
                                if (!fi.Name.StartsWith("plot"))
                                {
                                    Console.WriteLine($"{Now()} File not start with plot: {fi.Name}");
                                    continue;
                                }
                                ...
                                else if (fileName.EndsWith(_suffixM3))
                                {
                                    ...
                                    {
                                        Console.WriteLine($"{Now()} Destination file not exist: {destinationFullName}");
                                    }
                                }
                                else if (lastIgnoredFile == null || lastIgnoredFile != fileName)
                                {
                                    Console.WriteLine($"{Now()} Ignore file, don't ended with .plot: {fileName}");
                                    lastIgnoredFile = fileName;
